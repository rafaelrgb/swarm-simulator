...
function VO(robot, desiredVelocity) {
  ...
  // Atan2 returns angles in the interval (-pi, pi]
  ...
  //   if (theta_BA_left > Math.PI) theta_BA_left -= 2 * Math.PI;
  //   bound_left = [ Math.cos(theta_BA_left), Math.sin(theta_BA_left) ];
  //   theta_BA_right = theta_BA + theta_BA_ort;
  //   if (theta_BA_right <= -Math.PI) theta_BA_right += 2 * Math.PI;
...
function update() {
  ...
  for(var i = 0; i < robots.length; i++) {
    ...
    if(dist_to_goal >= bound) {
      ...
      robot.velocity = VO(robot, desiredVelocity);
    } else {
